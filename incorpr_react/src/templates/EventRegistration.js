    ...
    useEffect(() => {
        const fetchParticipants = async () => {
            try {
                ...
                console.log(response.data.users); // Логирование полученных данных
            } catch (err) {
        ...
        <div className="body-staff">
            ...
            <main className="container-staff">
                <div className="main-content-staff">
                    <h2>Участники <span>{title}</span></h2>
                                ...
                                <div className="employee-staff" key={participant.id}>
                                    <Link to={`/staff/${participant.id}`} className="link-staff">
                                        <div className="employee-info-staff">
                                            <img className="avatar-staff" src={`http://localhost:8080${participant.avatarUrl}`} alt={`avatar of ${participant.username}`} onError={(e) => e.target.src = 'http://localhost:8080/uploads/avatars/default avatar.jpg'} />
